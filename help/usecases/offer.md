...
[Node.js](https://nodejs.org/) is the programming platform. It comes with an enormous set of libraries, such as the Express web server. If you haven’t already, [download Node.js](https://nodejs.org/en/download/) and follow the steps to install this great open source development environment.
...
After signing into the [Adobe Developer Console](https://console.adobe.io/), click **Create New Project**. The project is named “Project 1” by default. Click the **Edit Project** button and change the name to “Offer Letter Generator.” In the center of the screen is a **Get Started With Your New Project** section. Take the following steps to enable security on your project:

Click **Add API**. This presents a number of APIs to choose from. In the **Filter by Product** section, select **Document Cloud**, then click **Next**.
...
Click the **Generate keypair** button. You get a config.zip file to download. Unzip the archive file. It contains two files: certificate_pub.crt and private.key. Make sure the latter is kept secure, as it contains your private credentials and may be used to generate spurious documents if out of your control.
...
You are asked some questions about your Node project. You can skip most of these, but make sure the project name is “letter-generator” and the entry point is **index.js**. Select **Yes** to complete project initialization.
...
There should be a new folder called node_modules added to your project. This is where all libraries (called dependencies in Node) are downloaded. The package.json file is also be updated with a reference to Adobe PDF Services.
...
As before, the dependencies section of package.json are updated accordingly.
...
Notice the get route returns an **index.html** file. Let’s create an HTML file with that name and the following simple form. You can add CSS styles and other design elements later as you see fit. This form will take the candidate’s basic details for generating a welcome letter:
...
You can make this model more elaborate if you wish, but for this tutorial, stick with this simple example. Also, note that there is no validation on this form as this is outside the scope of this article. To convert your form body to the data model described above, change the app.post handler method to have the following code:
...
The first line will put your JSON data in the desired format. Now you pass this data to a generateLetter function. Stop your server and paste the following code at the end of app.js. This code takes a Word document as a template and fills in placeholders with information from a JSON document.
...
Save the document as “OfferLetter-Template.docx” in a folder called “resources” in the root of your project. Notice the three Xs in the document. Those are temporary placeholders for your JSON information. Although you could use a special syntax to replace these, Adobe provides a Word Add-in that simplifies this task. Go to the Adobe [Document Generation Tagger Word Add-in](https://www.adobe.io/apis/documentcloud/dcsdk/docs.html?view=docgen-addin) site and follow the steps to install it.

In your OfferLetter-Template, click the new **Document Generation** button. A side panel will open. Click **Get Started**. You are provided with a text area to paste in the sample JSON data. Copy the “offer-data” snippet of JSON from above into the text area. It should look like the following:

![Screenshot of letter and code](assets/offer_3.png)

Click the **Generate Tags** button. You get a drop-down menu of tags to insert into the appropriate points in the document. Highlight the first X in the document and select **firstname**. Click **Insert Text** and “Dear X,” is changed to “Dear ```{{`offer_letter`.firstname}}```,”. This is the correct format for `documentMergeOperation`. Go ahead and add the remaining 3 tags at the appropriate Xs. Don’t forget to save OfferLetter-template.docx. It should look like this:
...
Now the Word template has markup that matches the JSON format. For example, ```{{`offer_letter`.`firstname`}}``` at the start of Word document will be replaced by the value in the "firstname" section of the JSON data.
...
* Copy the private.key file you generated earlier into the project and enter its name in the private_key_file section of the
 pdftools-api-credentials.json file. If you wish, you can put a path to the private key file here. Just remember to keep it secure as it can be mis-used once out of your control.
...
That’s it! Of course, this is just the beginning. If you study the Advanced section of the Document Generation tab of the Word Add-in, you will notice that not all the placeholder markers are from the associated JSON data. You can also add Signature tags. These tags allow you to take the resulting document and upload it to [Adobe Sign](https://acrobat.adobe.com/ca/en/sign.html) for delivery and signing to the new employee. Read Getting Started with Adobe Sign API to learn how to do it. This process is very similar as you will be using REST calls secured with a JWT token.

The single document example provided above can be used as the basis for an application where an organization needs to [ramp up seasonal hiring](https://www.adobe.io/apis/documentcloud/dcsdk/employee-offer-letters.html) of employees across multiple locations. As demonstrated, the main flow is to take data from candidates through an online application, use that data along with the job information to populate the fields of an offer letter, and send it out for electronic signature.
